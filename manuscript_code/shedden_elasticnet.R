...
load(file = "H:\\Courses\\cross_validation\\simulation\\data\\Shedden2008.RData")
...
# make tables 
C_Index <- matrix(NA, nrow = 4, ncol = 5)
Brier <- matrix(NA, nrow = 4, ncol =5)

load("H:\\Courses\\cross_validation\\simulation\\elasticnet_result_0.3.RData")
C_Index[,1] <- result[,4]
Brier[,1] <- result[,3]

load("H:\\Courses\\cross_validation\\simulation\\elasticnet_result_0.5.RData")
C_Index[,2] <- result[,4]
Brier[,2] <- result[,3]

load("H:\\Courses\\cross_validation\\simulation\\elasticnet_result_0.7.RData")
C_Index[,3] <- result[,4]
Brier[,3] <- result[,3]

load("H:\\Courses\\cross_validation\\simulation\\elasticnet_result_0.9.RData")
C_Index[,4] <- result[,4]
Brier[,4] <- result[,3]

load("H:\\Courses\\cross_validation\\simulation\\elasticnet_result_1.RData")
C_Index[,5] <- result[,4]
Brier[,5] <- result[,3]


# make figure
load("H:\\Courses\\cross_validation\\archive_sim\\elasticnet_result_0.3.RData")
range <- function(x){(x-min(x))/(max(x)-min(x))}
...
df <- rbind(
  ...
  data.frame(lambda = log(lambda[ID]), CVE = range(val_UG[ID]), Method = rep("Basic",length(ID))),
...
min <- group_by(df,Method)%>%summarise(lambdamin = lambda[which.min(CVE)])
 ...
 png(filename="H:\\Courses\\cross_validation\\manuscript_figure\\shedden.png", 
     units="px", 
     width=1500, 
     height=1000, 
     pointsize=15,
 ...
 # plot(x = log(lambda[ID]), y = range(val_GP[ID]), type = "l",col = 2,
 #      axes = FALSE,lwd = 2.5, xlab = expression(log(lambda)),
 #      ylab = "Standardized CVE",
 #      xlim = c(-0.4,-1.6), main = "elastic net, alpha = 0.3")
 # lines(x = log(lambda[ID]), y = range(val_UG[ID]), col = 3,lwd = 2.5)
 # lines(x = log(lambda[ID]), y = range(val_LP[ID]), type = "l",col = 4, lwd = 2.5)
 # lines(x = log(lambda[ID]), y = range(val_NA[ID]), type = "l", col = 6, lwd = 2.5)
 # axis(side = 1, at = seq(-1.6,-0.4,0.2),labels = seq(-1.6,-0.4,0.2))
 # axis(side = 2,las = 1)
 # legend("top",legend = c("V VH","Standard","Linear Pred","Dev Resid"),lty =1,col = c(2:4,6),lwd = 2)
dev.off()

tiff(filename="H:\\Courses\\cross_validation\\manuscript_figure\\shedden.tiff", 
    units="px", 
    width=1500, 
    height=1000, 
    pointsize=15,
