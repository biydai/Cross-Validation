...
#                         rho.noise=0, beta = 0.5,
#                         h = 1,
#                         cpct = 0.10,# 70% means 70% of the data are censored; 30% are observed
#                         corr = "autoregressive")
# coxph(data$y~data$x)
...
for(i in 1:N){
  ...
  for (j in 1:length(rho_all)){
    # generate data
    data <- genSurvData(n=n, p=p, a=nbeta, b=b, rho=rho_all[j], noise= 'autoregressive', beta = signal_s,
                        h = 1, cpct = 0.10,
                        corr = "autoregressive")
    data.test <- genSurvData(n=n, p=p, a=nbeta, b=b, rho=rho_all[j], noise= 'autoregressive', beta = signal_s,
                        h = 1, cpct = 0.10,
                        corr = "autoregressive")
    nevent <- sum(data$y[,2])
