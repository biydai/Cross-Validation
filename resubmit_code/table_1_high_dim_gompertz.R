...
genSurvData <- function(n = 100, beta = c(1,-1),cpct = 0.70,sd = 1,shape = 1, rate = 1 # 70% means 70% of the data are censored; 30% are observed
){
  p <- length(beta)
  ...
  hazard_rate <- shape*exp(x %*% beta)
  time <- eha::rgompertz(n, shape = hazard_rate, rate = rate, param = "rate")
...
for(i in 1:N){
  ...
  for (j in 1:length(c)){
    # generate data
    data <- genSurvData(n=n, beta = beta[j,], cpct = 0.3, sd = 1, shape = 1, rate = 2) #updated to gompertz
    data.test <- genSurvData(n=1000 , beta = beta[j,], cpct = 0, sd = 1, shape = 1, rate = 2) # updated to gompertz
    nevent <- sum(data$y[,2])
