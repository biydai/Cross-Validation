...
genSurvData <- function(n = 100, beta = c(1,-1),cpct = 0.70,sd = 1,scale = 1, shape = 1 # 70% means 70% of the data are censored; 30% are observed
){
  p <- length(beta)
  ...
  # use weibull regression
  hazard_rate <- (1/scale)*exp(x %*% beta/shape)
  time <- rweibull(n, shape = shape, scale = 1/hazard_rate)
...
for(i in 1:N){
  ...
  for (j in 1:length(c)){
    # generate data
    data <- genSurvData(n=n, beta = beta[j,], cpct = 0.3, sd = 1, shape = 1.5, scale = 1) #updated to weibull
    data.test <- genSurvData(n=1000 , beta = beta[j,], cpct = 0, sd = 1, shape = 1.5, scale = 1) # updated to weibull
    nevent <- sum(data$y[,2])
