...
\par We conducted simulations in both low dimensional ($p = 50$) and high dimensional ($p = 1000$) settings, varying the signal in the data \note{as in Section 3.1?} \note{is $n=100$ here?}. Results of the simulations are illustrated in Figure~\ref{Fig:cvauc}.  We used the linear predictor approach (CV-LP) here as the representative approach for likelihood-based cross-validation.
