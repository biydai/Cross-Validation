...
The results of this simulation are illustrated in the right panel of Figure~\ref{Fig:stability}. While the other three approaches are not affected by the number of folds and number of events in each fold, the variability of basic cross-validation increases dramatically as the number of events per fold decreases. When there are sufficient number of events in each fold, the basic approach is similar to the other three approaches. However, when there are only one or two events in each fold, the basic approach can be up to 10 times more variable than other methods. Thus, although the approach can be used to calculate CVE, there is considerable uncertainty surrounding its choice of the best model. The same phenomenon is also observed for the mean squared error. When there are only few events in each fold, MSE of the basic approach is affected and increases with the increasing uncertainty. 
...
Based on the simulation results illustrated in this section, we suggest that one should proceed with caution if they want to use the basic approach: i) always balance the censoring status when conducting fold assignments for the basic approach, ii) avoid using the basic approach in conjuction with high number of folds, iii) avoid using the basic approach when there are few number of events observed. Overall we would recommend using the linear predictor approach than the basic approach in practice.
...
Cross-validation is a crutial step in fitting penalized Cox regression models. In this paper, we propose two cross-validation methods for Cox Regression: cross-validated linear predictors and cross-validated deviance residuals. We conducted a series of simulation experiments to study and compare their performance with two existing approaches, the basic cross-validation and Verweij and Van Houwelingen's cross-validated partial likelihood, in selecting tuning parameters when fitting penalized regression models.
