...
They gathered expression profiles of 22,283 genes for 442 non-small-cell lung cancer (NSCLC) patients along with some additional clinical and demographic data. About $50\%$ of the patients died during the course of the study (236 events), with the rest censored. The clinical variables stage, age and gender were included in the model without penalization, as it is reasonable to assume that all three have an effect on survival. 

\begin{table}[h]
\centering
\begin{tabular}{ccccccccccc}
\toprule
 & \multicolumn{2}{c}{$\alpha = 0.3$ } & \multicolumn{2}{c}{$\alpha = 0.5$} & \multicolumn{2}{c}{$\alpha = 0.7$} & \multicolumn{2}{c}{$\alpha = 0.9$} & \multicolumn{2}{c}{$\alpha = 1$} \\
 &      $\lambda$      &   CVE         &   $\lambda$        &     CVE       &      $\lambda$      &     CVE       &    $\lambda$        &    CVE        &     $\lambda$      &    CVE       \\ 

\hline\\

 V$\&$VH  & 0.363 &  296.32  &  0.228   &  296.35  & 0.163 &  296.34  &  0.133  &   296.31  &  0.119  &  296.30  \\
 Standard & 0.301 & 145.17  & 0.218  &  145.38 & 0.163  & 145.37 &  0.127 &  145.35 & 0.114   & 145.34 \\
 Linear Pred & 0.288 & 10.661  & 0.181  & 10.673 & 0.163 & 10.673    & 0.127  & 10.672  &    0.114 &  10.672  \\
 Dev Resid & 0.316 & 571.40 & 0.228  &  571.96 & 0.163 & 571.95  & 0.133  &   571.79 &  0.119  &  571.70  \\
\bottomrule
\end{tabular}
\caption{\bnote{Not sure whether to include this table or not}}
\end{table}
...
\begin{tabular}{lcccccc}
  \toprule
  & $\lambda$ & log($\lambda$) & $\#$ of Genes Selected & CV C-Index & Brier Score \\ 
\cline{2-6} \\ 
V $\&$ VH& 0.363 & -1.013 &  & 0.639 & 0.243\\
standard & 0.301 & -1.200 &  & 0.647& 0.232\\
Linear Pred &0.288 & -1.246 &  & 0.650&  0.229\\
Dev Resid & 0.316 & -1.153 &  & 0.645& 0.235\\ 
\bottomrule
\end{tabular}
\caption{\label{tb:shedden}Results of elasticnet model with $\alpha = 0.3$ on the Lung Cancer Data Set. The linear Predictor approach picked the smallest $\lambda$. The corresponding model yields the highest CV C-Index and smallest Brier Score evaluated at median survvial time \bnote{I got different number of genes for LP approach...}}
\end{table}

For this analysis, we fit elastic net models to the data in addition to lasso models, as these models often offer improvement over the lasso when many correlated genes are involved \citep{Zou2005}. Besides the regularization parameter $\lam$, the elastic net model has an extra tuning parameter $\alpha$, where $\alpha=1$ corresponds to lasso and $\alpha=0$ corresponds to ridge regression. For each $\alpha = 0.3, 0.5, 0.7, 0.9 \text{ and } 1$, we selected a regularization parameter $\lam$ that has the lowest cross-validation error. The standard approach, linear predictor approach and deviance approach all reach the lowest cross-validation error at $\alpha = 0.3$. 

We report the predictive performance for all four models at $\alpha = 0.3$ and their corresponding $\lambda$s in Table~\ref{tb:shedden}. For each model, we computed the cross-validated C-Index and Brier Scores at predicting the median survival time. How the CV methods perform on real data reflects what is shown in simulation studies. The standard approach and the linear predictor approach, selecting smaller $\lambda$ values, were the more liberal, while Verweij and Van Houwelingen's CVE approaches were both more conversative. With the highest C Index and lowest Brier Score, the linear predictor approach has best predictive performance among all four methods. The same patterns are repeated when $\alpha$ is at different values.

We compared the CVE along the solution path for different methods. In Figure~\ref{fig:CVE}, the Cross Validated Error is rescaled and plotted for all four methods. A $\lambda$ is selected when CVE curve reaches its lower point. The blue line and green line, which represent the linear predictor approach and standard approah, have more curvature near theirs lowest point, making it is easier to pick out the minimum point. 

\begin{figure}[h]
  \centering
  \includegraphics[height= 8cm ]{./manuscript_figure/shedden.jpg}
  \caption{\label{fig:CVE}Rescaled Cross Validated Errors at $\alpha = 0.3$. The horizonal axis is $\log(\lambda)$. The vertical axis is the cross-validated error rescaled to a range from 0 to 1. After the rescaling, the Cross Validated Error of four different methods can be plotted together for comparison. }
\end{figure}


For the elastic net model with $\lam$ and $\alpha$ selected by the cross-validated linear predictor approach, 81 genes were selected.  Of those, 78 were well-annotated.  We carried out an enrichment analysis of these 78 genes using ToppGene \citep{Chen2009c}; the five most significantly enriched molecular functions are listed in Table~\ref{Tab:go}.

\begin{table}[ht]
\centering
\caption{\label{Tab:go} Five most significantly enriched molecular functions among genes selected by the linear predictor method with the elastic net ($\alpha=0.3$).  The five terms have a false discovery rate of 3\% using the Benjamini-Hochberg method. \note{I'm not sure whether we should include Selected and Annotated or not...if we do, we'll have to describe them better.}}
\begin{tabular}{llrrr}
\toprule
