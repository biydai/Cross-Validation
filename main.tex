...
\note{Biyue, I commented most of this section out, rather than deleting it, because it would probably make sense to move some of this to the supplement.  Can you please handle migrating this content to the supplement so that all the relevant equations/notation are defined?}

The fundamental challenge of conducting cross validation for Cox regression is that the baseline hazard is not estimated from the model, which prevents us from calculating prediction errors (residuals).  However, if one takes additional steps to estimate the baseline hazard in addition to estimating the coefficients, then such residuals can be calculated.  The deviance residual, a normalized form of the Martingale residual \citep{Therneau1990}, is a natural candidate for quantifying prediction error to use in quantifying the cross-validation error.   Originally proposed for model diagnostics, deviance residuals have also been occasionally used as a predictive accuracy measure \citep{Therneau2018}.  Given an estimate of the baseline cumulative hazard function, $\hat{\Lambda}_0(t)$, the cross-validated Martingale residual is given by
\begin{equation}
  m^{cv}_{i} = \delta_{i} - \hat{\Lambda}_{0}(t_{i})\exp(X_i\Tr\hat\beta^{-k}),
\end{equation}
and the cross-validated deviance residuals by
\begin{equation} 
  d_{i} = \text{sgn}(m^{cv}_{i})\sqrt{-2(\hat{M}^{cv}_{i} + \delta_{i}\log(\delta_{i} - \hat{M}^{cv}_{i}))}.
\end{equation}
...
In order to use this approach, of course, one must estimate the baseline hazard, and there are a variety of ways this may be accomplished.  We considered several approaches and found them to give more or less similar results, so in this manuscript, we present the baseline hazard estimation method that performed the best overall as the ``cross-validated deviance residuals'' approach.  Details of this method, the other methods we considered, and simulation results comparing these methods, are available in the supplemental materials.

%% The standard approach to estimating the cumulative baseline hazard, $\Lambda_0$, is the Nelson-Aalen Estimator \citep{nelson1969, aalen1978}:
%% \begin{equation}
...
\bnote{Simulations with features that has more complicated correlation structures were conducted as well. Similar patterns were observed as in Figure~\ref{Fig:mse-brier-c} and Table~\ref{Tab:sim}. Results of these simulations are provided in the supporting document.}

\subsection{\bnote{Power}}
\label{Sec:power}
