...
#############################################
# n = 200, p = 1000, # of none-zero beta = 20 #
#############################################
...
beta <- matrix(NA,nrow = length(c), ncol = p)
for (i in 1:length(c)){
  beta[i,]<- generatebeta(p,nbeta = 20 ,c = c[i])
}
N <- 200
# initiation
...
for(i in 1:N){
  ...
  for (j in 1:length(c)){
    ...
    n.l <- length(f$lambda)
    aic.ss <- smooth.spline(log(f$lambda),f$aic,df = 8)
    d <- diff(rev(aic.ss$y))
    if (all(d<0)){
      ind <- n.l
    }else {
    ...
    gcv.ss <- smooth.spline(log(f$lambda),gcv,df = 6)
    d <- diff(rev(gcv.ss$y))
    if (all(d<0)){
      ind <- n.l
    }else {
...
rownames(com_result1) <- c("true","ncvreg","aic","bic","gcv")

png(filename= "sim_4_high_lambda.png")
...
plot(x = c, y = com_result1[1,], type = "l",lwd = 2,main = maintitle,ylim = c(0,0.5),
...
legend("topright",legend = c("Optimal",
                             "Linear Pred","AIC","BIC","GCV"),lty = 1, col = 1:5)
dev.off()
...
rownames(mse_ratio) <- c("ncvreg","aic","bic","gcv")

png(filename= "sim_4_high_mse.png")
maintitle <- expression(paste("MSE Ratio"," vs the magnitude of non-zero ", beta, "s", sep = ""))
xlab <- expression(paste("the magnitude of non-zero ", beta, "s", sep = ""))
plot(x = c, y = mse_ratio[1,],ylim = c(0, 40), type = "l",lwd = 2,main = maintitle,
...
legend("topleft",legend = c("Linear Pred","AIC","BIC","GCV"),lty =1,col = 2:5)
dev.off()